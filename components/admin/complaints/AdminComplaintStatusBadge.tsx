...
export const AdminComplaintStatusBadge: FC<{ status: ComplaintStatus }> = ({
  ...
  const map: Record<ComplaintStatus, string> = {
    ...
    in_progress: "bg-amber-900/60 text-amber-200", // CHANGED: from in_review
  ...
  const labelMap: Record<ComplaintStatus, string> = {
    new: "New",
    in_progress: "In Progress", // CHANGED: from "In review"
    resolved: "Resolved",
    closed: "Closed",
    rejected: "Rejected",
  };

  return <span className={`${base} ${map[status]}`}>{labelMap[status]}</span>;
};